<template>
  ...
  >
    <div class="h-full flex justify-around items-center">
      ...
      >
        <Icon :icon="item.icon.id" class="text-2xl" />
...
const isVisible = computed(() => {
  if (!app?.isInstantiated) return false;
  if (!router.isReady()) return false;

  const route = router.currentRoute.value;

  const hasNav = (app?.content?.navigation?.drawer?.bottom?.length ?? 0) > 0;

  return hasNav && route.meta?.drawer;
});
